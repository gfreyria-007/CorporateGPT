...
(async () => {
  ...
  // After navigation, wait for the chat form to appear
  await page.waitForSelector('#chat-input-form', { state: 'visible', timeout: 300000 });
  // Target the input inside the form
  const chatInputSelector = '#chat-input-form input[data-testid="chat-input"]';
  await page.waitForSelector(chatInputSelector, { state: 'visible', timeout: 180000 });
